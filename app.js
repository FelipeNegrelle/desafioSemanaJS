...
const oddNumbers = randomNumbers.filter(e => e % 2 !== 0)
/*
  02
...
const lessThan = crazyNumbers.reduce((prev, e) => { return e < 501 ? prev + 1 : prev }, 0)
...
const squared = numbers.map(e=> {return e*e})
...
const old = tarantinoMovies.filter(( {release} ) => release < 2000)
...
// const gameNames = cart.map(game => {return console.log(`- ${game.name}\n`)}) //meu jeito
const gameNamesRoger = cart.reduce((accumulator, { name }) => `${accumulator}- ${name}\n`, '') //jeito Roger Melo

console.log(oddNumbers, lessThan, squared, old, names, gameNamesRoger)